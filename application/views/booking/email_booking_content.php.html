						...
						<?php if(has_special_offers($booking_item)):?>
							<tr>
								<td style="padding-left: 20px">
									...
									<?php if(!empty($booking_item['offer_note']) && !empty($booking_item['offer_name']) && !empty($booking_item['offer_cond'])):?>
									...
									<?php if(!empty($booking_item['promotion'])):?>
										<?php 
											$service_type = $booking_item['reservation_type'] == RESERVATION_TYPE_HOTEL ? HOTEL : TOUR;
											
										?>
										<span style="color: #FE8802;font-weight:bold;"><?=$booking_item['promotion']['name']?></span>
										<br>
										<?=load_promotion_content(false, $booking_item['promotion'], $service_type);?>
										<br>
									<?php endif;?>
