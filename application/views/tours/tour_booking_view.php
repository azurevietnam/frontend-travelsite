...
<form name="frm_delete_item" method="POST" action="/tour-booking/<?=$tour['url_title']?>.html">
...
<form name="frm" method="post" action="/tournext/<?=$tour['url_title']?>/<?=$parent_id?>/<?=$booking_from?>">

<input type="hidden" name="action_type"/>

<input type="hidden" name="back_url" value="<?=site_url('tour-booking/'.$tour['url_title'].'.html')?>"/>
		...
		<table class="book_service_table">
			...
			<?php 
				if ($booking_item['reservation_type'] != RESERVATION_TYPE_HOTEL){
					...
					$pax_accom_info = $booking_item['pax_accom_info'];
			...
			<?php endforeach;?>
				...
				<tr>
					<th colspan="3"><?=lang('transfer_services')?> (<?=lang('tour_booking_label_optional')?>)</th>
					<td class="bg_whiteSmoke"></td>
				</tr>
...
<?php else:?>
	<center>
		<strong><?=lang('tour_booking_message_web_timed_out')?></strong>
		<br><br>
		<a href="<?=url_builder(TOUR_DETAIL, $tour['url_title'], true)?>"><?=lang('message_click_here')?></a> <?=lang('tour_booking_message_back_to_chosen_tour')?>
	</center>
<?php endif;?>
...
<div style="float: right; margin: 0; margin-top: 20px;">
	...
	<?php 
		...
		$pax_accom_info = $main_item['pax_accom_info'];
		
		$num_pax = $booking_info['adults'] + $booking_info['children'];
		
		$booking_from = $this->uri->segment(3);
		
		$url_action = url_builder(TOUR_DETAIL, $tour['url_title'], true);
		
		if (!empty($booking_from) && $booking_from == "fromcruise" && isset($cruise)){
			
			$url_action = url_builder(CRUISE_DETAIL, $cruise['url_title'], true);
	...
	<form name="frm_check_rate" method="post" action="<?=$url_action?>">
		...
		<input type="hidden" name="tours" value="<?=$tour['id']?>"/>
		...
		<input type="hidden" name="departure_date_check_rates" value="<?=$booking_info['departure_date']?>">
		
		<input type="hidden" name="object_change" value="<?=$booking_info['object_change']?>">
		
		<?php if ($tour['cruise_id'] > 0 && !is_private_tour($tour)): // cruise tour?>
