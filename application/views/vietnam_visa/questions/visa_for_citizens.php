...
<h1 class="text-highlight"><?=lang_arg('need_passport_holder_question', ucfirst($country['name']))?></h1>
	<?php if($country['voa_accepted'] == 1 || $country['voa_accepted'] == 0):?>	
	<p class="price medium-font"><?=lang_arg('need_passport_holder_answer_yes', ucfirst($country['name']))?></p>
	...
	<ul class="margin-top-20 list-unstyled">
	  		...
	  		<li>
	  			...
	  			<div style="position: absolute; right: 10px; top: 50px">
  						...
  						<div class="btn btn-blue" onclick="javascript:window.frmApplyVisa.submit()">
	  						<?=lang('lb_apply_now')?>
						</div>
  				</div>
  				<input type="hidden" name="visa_req_nationality" value="<?=$country['id']?>">
  				<div class="btn btn-blue btn-lg" onclick="javascript:window.frmApplyVisa.submit()">
	  	...
	  	<ul style="line-height: 20px; list-style: square;color: #DF1F26">
